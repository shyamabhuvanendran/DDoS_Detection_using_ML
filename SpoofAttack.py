...
def pcapToCsv(fname):
    ...
    outfile='/Users/shyamabs/Documents/DDoD_ML_MS/datasets/training_data_spoof.csv'
...
fname = '/Users/shyamabs/Documents/DDoD_ML_MS/datasets/data_spoof.pcap'
#csvfile=pcapToCsv(fname)
csvfile='/Users/shyamabs/Documents/DDoD_ML_MS/datasets/training_data_spoof.csv'
...
#extracted_data  = pd.read_csv('/Users/shyamabs/Documents/DDoD_ML_MS/training_data_extracted_spoof.csv', index_col=0)
#print("extracted_data: ")
#print(extracted_data.head())

extracted_data.to_csv('/Users/shyamabs/Documents/DDoD_ML_MS/datasets/training_data_extracted_spoof.csv')
...
#poor_corr_cols = ["number_of_PA", "number_sport_35percent", "number_sport_75percent", "number_dport_20percent", "number_dport_35percent", "number_of_RA", "number_src_35percent", "number_src_50percent", "ratio_incom_to_outgo"]
...
CV_res_data = pd.DataFrame({"CVmeans":CV_means,"CVerrors": CV_std,"Algorithm":["SVC","RandomForest","GradientBoosting",
...
g = plot_learning_curve(gsMLP.best_estimator_,"Multiple Layer Perceptron learning curves",x_train,y_train,cv=kfold)
...
for row in range(nrows):
    for col in range(ncols):
        ...
        g = sns.barplot(y=x_train.columns[indices][:40],x = classifier.feature_importances_[indices][:40] > 0 , orient='h',ax=axes[row][col])
...
import pickle
pickle.dump(votingC, open('ensemble_model_spoof.sav', 'wb'))
#pickle.dump(votingC, open('ensemble_model_10batch_spoof.sav', 'wb'))
pickle.dump(GBC_best, open('choosen_model.sav', 'wb'))


loaded_model = pickle.load(open('ensemble_model_spoof.sav', 'rb'))
choosen_model = pickle.load(open('choosen_model.sav', 'rb'))
...
for i in block_indices:
    if x_test.loc[i]['ratio_ack_rst']<=26:
...
acc_combined = accuracy_score (y_pred_combined_df, y_test)
acc_combined

acc_spoof_spoofmodel = accuracy_score (y_pred, y_test)
acc_spoof_spoofmodel

y_pred_spoof_nonspoofmodel = non_spoof_model.predict(x_test)
acc_spoof_nonspoofmodel = accuracy_score (y_pred_spoof_nonspoofmodel, y_test)
acc_spoof_nonspoofmodel
...
xgb_model_exc_time = (time.time() - start_time)
print("xgb_model Execution Time:  %s seconds   " % xgb_model_exc_time)


start_time = time.time()
accuracy_score(loaded_model.predict(x_test), y_test)
loaded_model_exc_time = (time.time() - start_time)
print("loaded_model Execution Time:  %s seconds   " % loaded_model_exc_time)
# =============================================================================
# 
# 
# start_time = time.time()
# accuracy_score(ada_best.predict(x_test), y_test)
# ada_best_exc_time = (time.time() - start_time)
# print("ada_best Execution Time:  %s seconds   " % ada_best_exc_time)
# =============================================================================


start_time = time.time()
accuracy_score(RFC_best.predict(x_test), y_test)
RFC_best_exc_time = (time.time() - start_time)
print("RFC_best Execution Time:  %s seconds   " % RFC_best_exc_time)


start_time = time.time()
accuracy_score(GBC_best.predict(x_test), y_test)
GBC_best_exc_time = (time.time() - start_time)
print("GBC_best Execution Time:  %s seconds   " % GBC_best_exc_time)


start_time = time.time()
accuracy_score(SVMC_best.predict(x_test), y_test)
SVMC_best_exc_time = (time.time() - start_time)
print("SVMC_best Execution Time:  %s seconds   " % SVMC_best_exc_time)


start_time = time.time()
accuracy_score(ExtC_best.predict(x_test), y_test)
ExtC_best_exc_time = (time.time() - start_time)
print("ExtC_best Execution Time:  %s seconds   " % ExtC_best_exc_time)


start_time = time.time()
accuracy_score(choosen_model.predict(x_test), y_test)
choosen_model_exc_time = (time.time() - start_time)
print("Choosen Model Execution Time:  %s seconds   " % choosen_model_exc_time)
